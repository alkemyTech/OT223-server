...
describe('Testing s3Storage service', () => {
  ...
  describe('Testing getFilesFromURL', () => {
    ...
    test('sending unexistant file throws error', async () => {
      await expect(() => getFileFromURL('https://amazon.com/public/unexistant')).rejects.toThrow('URL provided does not exist');
    });
  ...
  describe('Testing deleteFileFromURL', () => {
    test('Fails to send not well formed URL', async () => {
      await expect(() => deleteFileFromURL('some wrong url 123')).rejects.toThrow('Invalid URL');
      await expect(() => deleteFileFromURL('https://amazon.com/')).rejects.toThrow('Invalid URL');
    });
